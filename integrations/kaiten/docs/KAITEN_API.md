...
Файлы к комментарию не прикладывать multipart-телом самого комментария — это старая загрузка с публичной ссылкой. Сначала создать комментарий, затем `POST /cards/{card_uid}/comments/{comment_uid}/files` (см. «Card Files»).
...
> **Загружать файлы — только через файлы с ограниченным доступом (маршруты по UUID ниже).** Старая загрузка (`PUT /cards/{card_id}/files`, multipart в комментариях) создаёт файл с постоянной публичной ссылкой, которую откроет кто угодно без авторизации, и отключается настройкой компании «Запретить загрузку файлов без ограниченного доступа через публичное API». Руководство Kaiten по миграции: https://developers.kaiten.ru/restricted-access-files-migration

### Restricted Access Files (файлы с ограниченным доступом)

Постоянной публичной ссылки у файла нет: права проверяются по родительской сущности, а на скачивание выдаётся временная подписанная ссылка. Ранее загруженные файлы автоматически не конвертируются — читать приходится оба поколения (см. «Старые файлы» ниже).
...
```bash
card_uid=$(curl -s "https://{domain}/api/latest/cards/{card_id}" \
  -H "Authorization: Bearer {token}" | jq -r '.uid')

curl -X POST "https://{domain}/api/latest/cards/${card_uid}/files" \
...
```bash
curl -s -L "https://{domain}/api/latest/cards/{card_uid}/files/{id}/content" \
  -H "Authorization: Bearer {token}" -o report.md
```

> **⚠️ Самая опасная ловушка миграции: загрузка не падает при неправильном идентификаторе.** `POST` с числовым `card_id` вернёт 200 — запрос уйдёт на старый маршрут и создаст обычный файл с постоянной публичной ссылкой. Проверять надо не код ответа, а то, что в пути стоит UUID и что в ответе `type` = `11`.
...
| Признак | Старый файл | Файл с ограниченным доступом |
|---------|--------------|------------------------------|
| `type` | `1` / `8` | `11` |
...
Маршруты по числовому `card_id` нужны только для чтения и удаления файлов, загруженных до миграции. **Для загрузки не использовать.**

| Метод | Endpoint | Описание |
|-------|----------|----------|
| GET | `/cards/{card_id}/files` | Список файлов карточки (оба поколения) |
| PATCH | `/cards/{card_id}/files/{id}` | Обновить файл (`card_cover`) |
| DELETE | `/cards/{card_id}/files/{id}` | Открепить файл |
| PUT | `/cards/{card_id}/files` | ~~Прикрепить файл~~ — создаёт публичный файл, отключается настройкой компании |
...
> **⚠️ Ссылка из `url` публична.** Замер: запрос к `https://files.kaiten.ru/<uuid>.<ext>` **без заголовка Authorization** возвращает 200 и сам файл. Ссылки на вложения нельзя класть в отчёты, spike-файлы и комментарии за пределами Kaiten. Скачивать старый файл — по `url` без токена.

> **⚠️ `403` на загрузке двусмысленный.** Кроме нехватки прав он означает, что компания включила запрет старой загрузки: тело `{"code": "PUBLIC_API_LEGACY_FILE_UPLOAD_DISABLED", "message": "Uploading files outside of restricted file access is disabled for the public API by company settings."}`. Запрет касается только multipart-загрузок (`POST|PUT /cards/{id}/files`, `POST|PUT /cards/{id}/comments` и `PATCH` комментария с `files[]`); правка текста комментария, переименование и удаление файла продолжают работать. Скрипты обязаны проверять HTTP-код: иначе прикрепление тихо не произойдёт. Увидели этот код — переходите на маршрут по `card_uid`, а не ищите обходной путь.
