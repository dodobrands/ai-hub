...
> ⚠️ **DEPRECATED.** Коннектор переехал в отдельный плагин `dodo-time` (MCP + OAuth):
> **https://hr-platform.dodois.io/#/ai-hub/dodo-time**
>
> Здесь ничего не удалено и всё продолжает работать — но новые задачи делай на `dodo-time`.
> Каталог удалим, когда команды сойдут с `time@ai-hub`.
>
> **Как переехать**
> 1. `claude plugin install dodo-time@dodo-ai-marketplace`
> 2. Перезапусти сессию — MCP-серверы подхватываются только при старте.
> 3. Вызови `time_whoami`. Первый вызов откроет браузер: корпоративный Google,
>    затем экран согласия Тайма. Копировать токены руками больше не нужно.
> 4. `claude plugin uninstall time@ai-hub`, затем убери `TIME_TOKEN` / `TIME_BOT_TOKEN`
>    из `.env` — но только если их не читает что-то ещё.
>
> **Важно:** `dodo-time` только читает. Отправки сообщений в нём нет намеренно —
> агент, который читает произвольные каналы и умеет в них писать, исполняет
> инструкции из чужих сообщений и пишет твоим токеном. Если нужна именно
> отправка, она пока остаётся здесь.
